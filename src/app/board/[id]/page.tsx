...
import { ArrowLeft, Check, Copy, ExternalLink, RefreshCw, X, FileDown } from 'lucide-react';
        ...
        if (node.type === "collection") {
          const newNode: CollectionNodeType = {
            ...
            data: {
              ...
              onDuplicate: (nodeData: CollectionNodeData) => {
                setSelectedCollection(node.id);
                selectedCollectionRef.current = node.id;
                handleNodeDuplicate(nodeData);
              },
              onRename: (id: string) => {
                const node = (data.draw.flow.nodes as NodeType[]).find(n => n.id === id);
                console.log("RENAME", id, nodesLocal)
  ...
  return <>
    ...
    <Dialog open={shareDialogOpen} onOpenChange={(e) => setShareDialogOpen(e)}>
      <DialogContent className='border-2 border-white/20 text-white'>
        ...
        <input disabled value={window.location.origin + `/diagram/${params.id}`} className='input' />
        <DialogFooter>
          <button onClick={() =>{
            navigator.clipboard.writeText(`${window.location.origin}/diagram/${params.id}`);
            toast.success('Link copied to clipboard');
            setCopied(true);
            setTimeout(() => setCopied(false), 2000);
          }} className='bg-blue-600 button w-full flex items-center gap-1 justify-center'>{copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}{copied ? "Copied!" : "Copy Link"}</button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <Dialog open={nodeRenameDialogOpen.show} onOpenChange={(e) => {
      if (!e) {
        setRenameNode("");
        setNodeRenameDialogOpen({ show: false, id: "" });
      }
    }}>
      <DialogContent className='border-2 border-white/20 text-white'>
        <DialogHeader>
          <DialogTitle className='text-white'>Rename Collection</DialogTitle>
          <p className='text-white/60 text-sm mt-1.5'>Enter a new name for this collection</p>
        </DialogHeader>
        <form onSubmit={(e) => {
          e.preventDefault();
          if (renameNode.trim()) {
            setNodes((prevNodes) => prevNodes.map(node => {
              if (node.id === nodeRenameDialogOpen.id && node.type === 'collection') {
                return {
                  ...node,
                  data: {
                    ...node.data,
          ...
          <DialogFooter className='mt-4'>
            <button 
              type='submit' 
              disabled={!renameNode.trim()} 
              className='bg-blue-600 button w-full disabled:opacity-50'
            >
              Rename
            </button>
          </DialogFooter>
